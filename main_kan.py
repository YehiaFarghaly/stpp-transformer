...
from model_kan import KANTransformer
import pandas as pd
import torch
...
model = KANTransformer(src_input_dim, d_model, N, h, d_ff, dropout, src_seq_len, latent_dim, kan_layers)
...
for epoch in range(epochs):
    ...
    if avg_val_loss < best_val_loss:
        ...
        patience_counter = 0
        torch.save(model.state_dict(), 'best_transformer_model_kan.pth')
    else:
        patience_counter += 1
...
model.load_state_dict(torch.load('best_transformer_model_kan.pth'))
model.eval()