...
def generate_event_thinning(event_data, T, model, spatial_range):
    ...
    for i in range(batch_size):
        ...
        # Predict dynamic Hawkes process parameters using the model
        mu_seq, alpha_seq, beta_seq, gamma_seq = model(event_data[i].unsqueeze(0), None)
        
        print('mu: ', mu_seq)
        print('beta: ', beta_seq)
        print('alpha: ', alpha_seq)
        print('gamma: ', gamma_seq)
        while current_time < T[i].item():
            # Use the parameters corresponding to the last event
            mu = mu_seq[:, -1, 0].item()
            alpha = alpha_seq[:, -1, 0].item()
            beta = beta_seq[:, -1, 0].item()
            gamma = gamma_seq[:, -1, 0].item()
...
class Decoder(nn.Module):
    ...
    def forward(self, x, src_mask):
        mu = F.softplus(self.fc_mu(x))  # Output shape: (batch_size, seq_len, latent_dim)
        alpha = F.softplus(self.fc_alpha(x))
        beta = F.softplus(self.fc_beta(x))
        gamma = F.softplus(self.fc_gamma(x))
...
class Transformer(nn.Module):
    ...
    def log_likelihood(self, event_times, event_locations, mu_seq, alpha_seq, beta_seq, gamma_seq, target_times, target_locations, dt=0.1, dx=0.1, dy=0.1):
        log_intensity_sum = 0
        ...
        for batch_idx in range(target_times.shape[0]):
            ...
            batch_mu_seq = mu_seq[batch_idx]
            batch_alpha_seq = alpha_seq[batch_idx]
            batch_beta_seq = beta_seq[batch_idx]
            batch_gamma_seq = gamma_seq[batch_idx]
            ...
            for i in range(len(batch_target_times)):
                ...
                # Use the parameters corresponding to the current target event
                mu = batch_mu_seq[i]
                alpha = batch_alpha_seq[i]
                beta = batch_beta_seq[i]
                gamma = batch_gamma_seq[i]
            ...
            for t in time_grid:
                for x in x_grid:
                    for y in y_grid:
                        ...
                        # Use the parameters at the current grid time index (you might need to interpolate or select the closest parameter)
                        mu = batch_mu_seq[i]
                        alpha = batch_alpha_seq[i]
                        beta = batch_beta_seq[i]
                        gamma = batch_gamma_seq[i]
        ...
        return log_intensity_sum - integrated_intensity_sum
